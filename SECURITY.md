...
- 登录失败统一返回相同错误；同一来源在 1 分钟内连续失败 5 次后会被限制 60 秒，响应包含 `Retry-After` 和剩余秒数，记录会过期并在容量达到上限时按最近使用情况淘汰
...
- 管理面板默认监听 `127.0.0.1`；如需公网绑定必须显式设置 `PANEL_BIND_ADDR`，并启用 HTTPS 或显式设置 `ALLOW_INSECURE_HTTP=true`
...
- SQLite 数据库文件包含管理员密码哈希和站点配置；固定上游 Header 的名称以明文保存，值使用独立的 `UPSTREAM_HEADER_KEY` 通过 AES-GCM 加密；Cloudflare DNS API Token 和 Telegram Bot Token 使用独立的 `MERIDIAN_SECRET_KEY` 加密
- 管理 API 对固定上游 Header 采用只写语义，不回显明文或数据库密文。`UPSTREAM_HEADER_KEY` 不得与 `JWT_SECRET` 共用，且必须随数据库一起备份；`MERIDIAN_SECRET_KEY` 也必须与数据库一起备份，否则已保存的外部凭据无法解密
