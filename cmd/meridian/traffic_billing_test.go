...
func TestTrafficBillableBytesCountsEachPayloadDirectionOnce(t *testing.T) {
	videoBytes := int64(784) * (1 << 30) / 100
	if got := trafficBillableBytes(trafficBillingModeBidirectional, 0, videoBytes); got != videoBytes {
		t.Fatalf("bidirectional 7.84 GiB video = %d, want %d", got, videoBytes)
	}
	...
	if got := trafficBillableBytes(trafficBillingModeBidirectional, 2, 3); got != 5 {
		t.Fatalf("bidirectional 2-byte upload plus 3-byte download = %d, want 5", got)
	}
}

func TestTrafficBillableBytesClampsInvalidAndOverflowingCounters(t *testing.T) {
	if got := trafficBillableBytes(trafficBillingModeBidirectional, -10, 5); got != 5 {
		t.Fatalf("negative input clamp = %d, want 5", got)
	}
